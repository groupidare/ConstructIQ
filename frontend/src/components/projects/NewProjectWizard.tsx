  ...
  async function onSubmit(data: FormData) {
    try {
      const created = await createProject({
        ...
        budget: 0,
        status: data.isHistorical ? 'Completed' : undefined,
        phases: [],
      });
      toast.success(
        data.isHistorical
          ? 'Historical project created — now enter its phases and actual quantities used.'
          : 'Project created — now add phases, measurements & material plan.'
      );
      setProject(created);
      setStep(2);
    } catch {
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Step 2 of 2 — Measurements &amp; Material Plan</p>
        </div>
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <Card>
          ...
          <CardBody className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <label className="md:col-span-2 flex items-start gap-2 text-sm text-gray-700">
              ...
              <span>
                This is a <strong>completed project</strong> — enter it as historical data
                (actual quantities used) to help train the forecasting model.
              </span>
            </label>
          </CardBody>
        </Card>
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button type="submit" loading={isSubmitting}>Next: Measurements &amp; Material Plan</Button>
        </div>
      </form>
    </div>
  );
